...
\begin{tabular}{|c|l|}
\hline
\textbf{Variable} & \multicolumn{1}{c|}{\textbf{Definition}}                     \\ \hline
...
$a_i$             & Probability of final state being Y, given initial state is i \\ \hline
\end{tabular}
}
\caption{Definition of different variables}
\label{table:1}
\end{table}
\\When a fair coin is tossed twice the output is, \\
'N' when the outcomes are (Head, Head),\\
(Head, Tail) and\\
'Y' when the outcome is
(Tail, Head).
Then, \\
...
\textbf{Given :} Initial state is 'S'\\
\textbf{To find :} Probability of outcome being Y.\\
Therefore we have to find the probability of final state being state Y.\\
